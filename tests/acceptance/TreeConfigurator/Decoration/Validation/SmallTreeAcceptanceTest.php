...
class SmallTreeAcceptanceTest extends TestCase
{
    ...
    {
        $builtTree = $builtTreeProvider->get('tree:small');
    ...
    {
        self::assertCount(4, $builtTree->getRows());
    ...
    public function testRowOnlyNestedFeatureSymbols(BuiltTreeInterface $builtTree) : BuiltTreeInterface
    {
        ...
        $knownValids = [ ...$treeDecorations['small'], ...$treeDecorations['medium'] ];
        $knownInvalids = $treeDecorations['big'];

        foreach (['row0', 'row1', 'row2', 'row3'] as $rowId) {
